...
Այս գրքում իրականացնում ենք BASIC[^1] լեզվի մի տարատեսակ, որին տվել ենք BASIC-IR անունը։ IR մասնիկը հուշում է, որ իրականացվող կոմպիլյատորը BASIC ծրագիրը թարգմանում է LLVM համակարգի IR[^2] ներկայացման։ Լեզուն ունի երեք ներդրված տիպ՝ _իրական_ (`REAL`), _տրաբաբանական_ (`BOOLEAN`) և _տեքստային_ (`TEXT`)։ Իրական արժեքների հետ կարելի է կատարել թվաբանական ու համեմատման գործողություններ։ Տեքստային արժեքների հետ կարելի է կատարել միակցման (concatenation) գործողությունը։ Տրամաբանական արժեքների հետ՝ կոյունքցիայի, դիզյունքցիայի, ժխտման, ինչպես նաև հավասարության ու անհավասարության ստուգման գործողությունները։ Փոփոխականի տիպը որոշվում է նրա անվան կառուցվածքով. եթե իդենտիֆիկատորն ավարտվում է `$` նիշով, օրինակ, `a$` կամ `se$`, ապա այն `TEXT` տիպի է։ Եթե անունն ավարտվում է `?` նիշով, ապա այն տրամաբանական տիպի է։ Բոլոր այլ դեպքերում `REAL`: Տիպերը խստորեն ստուգվում են իմաստաբանական վերլուծության ժամանակ, իսկ անհամապատասխանությունների դեպքում կոմպիլյատորը հայտնում է սխալի մասին։ Փոփոխականը սահմանվում է և արժեք է ստանում _վերագրման_ `LET` հրամանով։ Օրինակ.
...
' եթե a > b, ապա ...
IF a > b THEN
  ' a-ն վերագրվում է out փոփոխականին
  LET out = a
END IF
```

Մեր լեզուն չի նախատեսում տվյալների ներմուծման ու արտածման հրամաններ, ուստի ծրագրի արդյունքներն օգտակար է պահպանել փոփոխականներում։ Օրինակ, եթե պահանջվում է նվազման կարգով հաշվել 1..100 միջակայքի զույգ թվերի քառակուսիները, ապա կարելի է `FOR` ցիկլի կիրառությամբ գրել.
...
Որպես ծրագրի կազմակերպման միավոր BASIC-IR լեզուն թույլատրում է սահմանել _ենթածրագրեր_ (subroutine)։ BASIC-IR ծրագրում ենթածրագրերը կարող են կատարել և՛ պրոցեդուրայի, և՛ ֆունկցիայի դեր։ Օրինակ, `Area` պրոցեդուրան `r` շառավղով շրջանի մակերեսը պահում է `ca` փոփոխականում։
...
## BASIC-IR շարահյուսությունը

BASIC-IR լեզվի նկարագրությունը սկսենք ամենախոշոր միավորներից՝ ծրագրից ու ենթածրագրից։ Այս դեպքում մեր ամեն մի քայլում սահմանելու ենք նախորդ շարահյուսական կանոնի մեջ օգտագործված բայց դեռ չսահմանված ոչ տերմինալային սիմվոլները։ Կարելի է, իհարկե, լեզվի շարահյուսությունը սահմանել «մանրերից դեպի խոշորները» սկզբունքով։ Այդ դեպքում էլ ամեն մի քայլում քերականական հավասարումը կառուցվում է նախորդ քայլերում արդեն սահմանված ոչ տերմինալներից ու տերմինալներից։
